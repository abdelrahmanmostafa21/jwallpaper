...
class _UpdateConfig extends ThemeEvent with AppLogger {
  const _UpdateConfig({
...
class _ChangeTheme extends ThemeEvent with AppLogger {
  const _ChangeTheme({
    this.lightThemeId,
    this.darkThemeId,
  ...
  AppThemeData? _themeDataFromId(int? id, ThemeState state) {
    if (id == null) {
      return null;
    }
  ...
  Future<void> handle(ThemeBloc bloc, Emitter emit) async {
    if (lightThemeId != null) {
      ...
      if (saveSelection) {
        di<ThemeDatasource>().lightThemeId = lightThemeId!;
      }
    }
    ...
    if (darkThemeId != null) {
      ...
      if (saveSelection) {
        di<ThemeDatasource>().darkThemeId = darkThemeId!;
      }
    }
...
class _LoadCustomThemes extends ThemeEvent with AppLogger {
  const _LoadCustomThemes();

  AppThemeData? _decodeThemeData(String themeDataJson) {
    try {
      return AppThemeData.fromJson(jsonDecode(themeDataJson));
    } catch (e, st) {
  ...
  Future<void> handle(ThemeBloc bloc, Emitter emit) async {
    ...
    final customThemesData =
        di<ThemeDatasource>().customThemes.map(_decodeThemeData).whereType<AppThemeData>().toList();
...
class _DeleteCustomTheme extends ThemeEvent with AppLogger {
  ...
  Future<void> handle(ThemeBloc bloc, Emitter emit) async {
    ...
    if (index >= 0 && index < customThemesData.length) {
      ...
      final lightThemeId = di<ThemeDatasource>().lightThemeId;
      final darkThemeId = di<ThemeDatasource>().darkThemeId;
...
class _AddCustomTheme extends ThemeEvent with AppLogger {
...
void _persistCustomThemes(List<AppThemeData> customThemesData) {
  ...
  di<ThemeDatasource>().customThemes = encodedCustomThemes;
}
